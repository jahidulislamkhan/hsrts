...
@section('main')
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Repair Invoice</li>
        </ol>
    </section>
            ...
            <div class="col-md-12">
                                    ...
                                    <table class="table" style="margin: 0">
                                        <tr>
                                            <td style="border-top: none;">
                                                ...
                                                <p>Shop : {{ $invoice->shop_id == 0 ? 'In House' : ucwords($invoice->shop->name) }} <br />
              ...
              <span class="small-text">
                {{ $invoice->delivery_date->format('jS M Y') }} <br>
                                                    ...
                                                    <select name="status">
                                                        <option value="waiting" {{ $invoice->status== 'waiting'? 'selected' : '' }}> Waiting for repairment </option>
                                    ...
                                    <table class="table"  style="margin: 0">
                                        <tr>
                                            ...
                                            <address class="small-text">
                                                {{ $invoice->shop_id == 0 ? 'In House' : $invoice->shop->name }} <br />
                                                {!! $invoice->address != ''? nl2br($invoice->address).'<br />' : '' !!}
                                                Contact : demo345 <br />
                                                E-mail : {{ $invoice->shop_id == 0 ? 'support@hsrts.com' : $invoice->shop->email }}
                                            </address>
                                            </td>
...
@section('page-js')
    <script>
        $('select[name="status"]').change(function(){
            ...
            $('span#processing').html('<i class="fa fa-spinner fa-spin"></i>').fadeIn(6000);
            ...
            $.ajax({
                ...
                success : function(data){
                    if(data.status == 1){
                        $('span#processing').html('').fadeOut(3000).delay(10000, function(){
                            $('span#processing').css('display', 'inline');
                        });
                    }else {
